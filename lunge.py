...
TRANSLATIONS = {
    ...
    "down": "вниз",
    "COUNT": "СЧЕТ",
    "KNEE_OVER_TOE": "КОЛЕНО ЗА НОСКОМ",
    "Correct": "Правильно",
...
def analyze_knee_angle(
    ...
    if draw_to_image is not None and stage != "down":
        ...
        # Visualize angles
        # Visualize angles with Russian labels
        cv2.putText(
            image,
            f"Угол: {int(results['right']['angle'])}",  # "Angle" in Russian
        ...
        cv2.putText(
            image,
            f"Угол: {int(results['left']['angle'])}",  # "Angle" in Russian
    ...
    if draw_to_image is not None:
        ...
        # If there is an error, display the error message in Russian
    if results["error"]:
        cv2.rectangle(image, (0, 50), (200, 100), (245, 117, 16), -1)
        cv2.putText(
            image,
            "ОШИБКА УГЛА КОЛЕНА",  # "KNEE ANGLE ERROR" in Russian
            (10, 62),
            cv2.FONT_HERSHEY_COMPLEX,
            0.3,
            (0, 0, 0),
        ...
        cv2.putText(
            image,
            "ЛЕВОЕ КОЛЕНО" if results["left"]["error"] else "ПРАВОЕ КОЛЕНО",  # "LEFT KNEE" or "RIGHT KNEE"
            (10, 82),
            cv2.FONT_HERSHEY_COMPLEX,
            0.3,
            (255, 255, 255),
...
class LungeDetection:
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            # Translate current stage dynamically
            self.current_stage = TRANSLATIONS.get(self.current_stage, self.current_stage)
            ...
            err_prediction_probability = None
                        # Translate stage_predicted_class dynamically
            translated_stage_predicted_class = TRANSLATIONS.get(stage_predicted_class.split(" ")[0], stage_predicted_class)

            # Translate k_o_t_error dynamically
            translated_k_o_t_error = TRANSLATIONS.get(k_o_t_error, k_o_t_error)
            ...
            cv2.putText(
                image,
                f'{str(self.counter)}, {TRANSLATIONS.get(stage_predicted_class.split(" ")[0], stage_predicted_class)}, {str(stage_prediction_probability)}',
                (5, 30),
                cv2.FONT_HERSHEY_COMPLEX,
                0.5,
                (255, 255, 255),
            ...
            cv2.putText(
                image,
                TRANSLATIONS["KNEE_OVER_TOE"],  # "КОЛЕНО ЗА НОСКОМ"
                (145, 12),
                cv2.FONT_HERSHEY_COMPLEX,
                0.5,
                (0, 0, 0),
            ...
            cv2.putText(
                image,
                f"{TRANSLATIONS.get(err_predicted_class, err_predicted_class)}, {err_prediction_probability}, {TRANSLATIONS.get(k_o_t_error, k_o_t_error)}",
                (135, 30),
                cv2.FONT_HERSHEY_COMPLEX,
                0.5,
                (255, 255, 255),
