...
TRANSLATIONS = {
    "unknown": "неизвестно",
    "correct": "правильно",
    "too tight": "слишком узко",
    "too wide": "слишком широко",
    "down": "вниз",
    "up": "вверх",
    "middle": "середина",
}
...
class SquatDetection:
    ML_MODEL_PATH = "model/squat_model.pkl"
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            # Translate dynamically generated text
            ...
            feet_placement_russian = TRANSLATIONS.get(feet_placement, feet_placement)
            knee_placement_russian = TRANSLATIONS.get(knee_placement, knee_placement)
            predicted_class_russian = TRANSLATIONS.get(predicted_class, predicted_class)
            # Status box
            # Draw a status box with a larger width for better balance
            cv2.rectangle(image, (0, 0), (420, 50), (245, 117, 16), -1)

            # Display "СЧЕТЧИК" (Counter)
            cv2.putText(
                image,
                "СЧЕТЧИК",
                (10, 15),
            ...
            cv2.putText(
                image,
                f'{str(self.counter)}, {predicted_class_russian}, {str(prediction_probability)}',
                (10, 35),
                cv2.FONT_HERSHEY_COMPLEX,
                0.5,
                (255, 255, 255),
