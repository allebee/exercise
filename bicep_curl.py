...
TRANSLATIONS = {
    ...
    "up": "вверх",
    "correct": "правильно",
    "loose upper arm": "расслабленная верхняя часть руки",
    "peak contraction": "слабое сокращение",
    "error": "ошибка",
    "unknown": "неизвестно",
}
...
class BicepPoseAnalysis:
    ...
    def analyze_pose(
        ...
        if ground_upper_arm_angle > self.loose_upper_arm_angle_threshold:
            has_error = True
            cv2.rectangle(frame, (350, 0), (650, 40), (245, 117, 16), -1)
            cv2.putText(
                frame,
                "ОШИБКА РУК",
                (360, 12),
...
class BicepCurlDetection:
    ML_MODEL_PATH = "model/bicep_curl_model.pkl"
    INPUT_SCALER = "model/bicep_curl_input_scaler.pkl"
    ...
    def detect(
        ...
        try:
            ...
            # Adjusted status box dimensions
            status_box_width = 350
            status_box_height = 80
            cv2.rectangle(image, (0, 0), (status_box_width, status_box_height), (245, 117, 16), -1)

            # Padding values for consistent alignment
            padding_x = 10
            line_height = 20
            ...
            cv2.putText(
                image,
                "ПРАВАЯ РУКА",  # "RIGHT ARM" in Russian
                (padding_x, line_height),  # First row
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (0, 0, 0),
                1,
            ...
            cv2.putText(
                ...
                else "НЕИЗВ.",  # "UNKNOWN" in Russian
                (padding_x + 150, line_height),  # Aligned next to label
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (255, 255, 255),
                1,
            ...
            cv2.putText(
                image,
                "ЛЕВАЯ РУКА",  # "LEFT ARM" in Russian
                (padding_x, line_height * 2),  # Second row
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (0, 0, 0),
                1,
            ...
            cv2.putText(
                ...
                (padding_x + 150, line_height * 2),  # Aligned next to label
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (255, 255, 255),
                1,
            ...
            # Lean Back Error Status
            cv2.putText(
                image,
                "ОТКЛОНЕНИЕ НАЗАД",  # "LEAN-BACK" in Russian
                (padding_x, line_height * 3),  # Third row
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (0, 0, 0),
                1,
            ...
            cv2.putText(
                image,
                "ОШИБКА" if self.stand_posture == "L" else "ПРАВ",  # "ERROR" or "CORRECT"
                (padding_x + 250, line_height * 3),  # Aligned next to label
                cv2.FONT_HERSHEY_COMPLEX,
                0.6,
                (255, 255, 255),
                1,
            ...
            # Display Angles at Bottom Corners
            if self.left_arm_analysis.is_visible:
                cv2.putText(
                    image,
                    f"Угол: {left_bicep_curl_angle}",  # "Angle" in Russian
                    (padding_x, image.shape[0] - 60),  # Bottom-left
                    cv2.FONT_HERSHEY_COMPLEX,
                    0.5,
                    (255, 255, 255),
                    1,
                ...
                cv2.putText(
                    image,
                    f"Угол плеча: {left_ground_upper_arm_angle}",  # "Shoulder Angle" in Russian
                    (padding_x, image.shape[0] - 40),  # Bottom-left
                    cv2.FONT_HERSHEY_COMPLEX,
                    0.5,
                    (255, 255, 255),
                    1,
            ...
            if self.right_arm_analysis.is_visible:
                cv2.putText(
                    image,
                    f"Угол: {right_bicep_curl_angle}",  # "Angle" in Russian
                    (image.shape[1] - 200, image.shape[0] - 60),  # Bottom-right
                    cv2.FONT_HERSHEY_COMPLEX,
                    0.5,
                    (255, 255, 255),
                    1,
                ...
                cv2.putText(
                    image,
                    f"Угол плеча: {right_ground_upper_arm_angle}",  # "Shoulder Angle"
                    (image.shape[1] - 200, image.shape[0] - 40),  # Bottom-right
                    cv2.FONT_HERSHEY_COMPLEX,
                    0.5,
                    (255, 255, 255),
                    1,
