...
TRANSLATIONS = {
    "correct": "правильно",
    "low back": "низкая спина",
    "high back": "высокая спина",
    "unknown": "неизвестно",
}
...
class PlankDetection:
    ML_MODEL_PATH = "model/plank_model.pkl"
    INPUT_SCALER_PATH = "model/plank_input_scaler.pkl"
    ...
    def load_machine_learning_model(self) -> None:
        ...
        if not self.ML_MODEL_PATH or not self.INPUT_SCALER_PATH:
            raise Exception("Cannot find plank model file or input scaler file")
    ...
    def detect(self, mp_results, image, timestamp) -> None:
        ...
        try:
            ...
            # Translate current stage
            current_stage_russian = TRANSLATIONS.get(current_stage, current_stage)
            ...
            cv2.putText(
                image,
                "ВЕРОЯТНОСТЬ",  # "PROBABILITY" in Russian
                (10, 20),
            ...
            cv2.putText(
                image,
                current_stage_russian,  # Translated stage
                (150, 50),
                cv2.FONT_HERSHEY_COMPLEX,
                0.5,
                (255, 255, 255),
